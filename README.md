...
> Current development version: **0.56.0**. In `TESTNET` mode, PAPER strategies continue collecting statistics while one isolated Binance Spot Testnet Grid bot trades virtual funds. The dashboard uses Testnet-native pricing and candles, localized compact order diagnostics, explicit environment labels, manual/stale synchronization controls, and batched Telegram fill summaries; LIVE execution remains disabled.
